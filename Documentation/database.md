...
```mermaid
erDiagram
  Fact_Vuelos }o--|| Dim_Aerolinea : opera
  Fact_Vuelos }o--|| Dim_Aeropuerto : origen
  Fact_Vuelos }o--|| Dim_Aeropuerto : destino
  Fact_Vuelos }o--|| Dim_Aeronave : usa
  Fact_Vuelos }o--|| Dim_ClaseCabina : vende
  Fact_Vuelos }o--|| Dim_Pasajero : transporta
  Fact_Vuelos }o--|| Dim_CanalVenta : vendido_por
  Fact_Vuelos }o--|| Dim_MetodoPago : pagado_con
  Fact_Vuelos }o--|| Dim_EstadoVuelo : estado
  Fact_Vuelos }o--|| Dim_Fecha : sale_en
  Fact_Vuelos }o--|| Dim_Fecha : reservado_en

  Fact_Vuelos {
    bigint vuelo_key PK
    int record_id "clave de negocio (fuente CSV)"
    varchar numero_vuelo "dimension degenerada"
    varchar asiento "dimension degenerada"
    ...
    decimal precio_ticket_original
    char moneda_original
    decimal precio_ticket_usd
    tinyint maletas_total
    tinyint maletas_facturadas
  }
  Dim_Aerolinea {
    int aerolinea_key PK
    varchar codigo_iata UK
    varchar nombre
  }
  Dim_Aeropuerto {
  ...
  Dim_ClaseCabina {
    int clase_cabina_key PK
    varchar nombre_clase UK
  }
  Dim_Pasajero {
    int pasajero_key PK
    uuid passenger_id UK "clave de negocio"
    char genero
    smallint edad
    char nacionalidad
  }
  Dim_CanalVenta {
    int canal_venta_key PK
    varchar nombre_canal UK
  }
  Dim_MetodoPago {
    int metodo_pago_key PK
    varchar nombre_metodo UK
  }
  Dim_EstadoVuelo {
    int estado_vuelo_key PK
